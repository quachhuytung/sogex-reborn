...
class FacebookProfileSpider(FacebookCore):
    ...
    def crawl_timeline(self, response, timeline_n_times):
        posts_link_container = LinkExtractor(restrict_xpaths=\
                config_data['facebook']['fanpage']['timeline_page']['post_link_xpath'])\
    ...
    def parse_post_content(self, response, comment_list_n_times):
        ...
        post_loader.add_xpath('content', config_data['facebook']['fanpage']\
        ['post_page']['contents_container_xpath'])
        post_loader.add_xpath('id', config_data['facebook']['fanpage']\
        ['post_page']['id_xpath'])
        ...
        comments_container = response.xpath(config_data['facebook']['fanpage']\
        ...
        for comment_container in comments_container[:-1]: # last one is for next page
            ...
            comment_loader.add_value('id', comment_container.xpath(\
                config_data['facebook']['fanpage']['post_page']['comment_id_relative_xpath'])\
                .get())
            ...
            comment_loader.add_value('author_name', \
                comment_container.xpath(config_data['facebook']['fanpage']['post_page']\
        ...
        if comment_list_n_times < self.max_scroll_comment:
            next_comment_page_url_container = LinkExtractor(\
                restrict_xpaths=config_data['facebook']['fanpage']['post_page']\
    ...
    def parse_reaction_page(self, response, obj_id, obj_type):
        reactions_label = response.xpath(config_data['facebook']['fanpage']['reaction_page']\
        ['labels_xpath']).getall()
        reactions_data = response.xpath(config_data['facebook']['fanpage']['reaction_page']\
        ['data_xpath']).getall()
